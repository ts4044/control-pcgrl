...
class Minecraft2DmazeProblem(Problem):
    ...
    def get_stats(self, map):
        ...
        self.path_length, self.path_coords = calc_longest_path(
            map, map_locations, ["AIR"], get_path=self.render_path
        )
        return {
    ...
    def render(self, map):
        if self._graphics == None:
            self._graphics = {
                "AIR": Image.open(
                    os.path.dirname(__file__) + "/minecraft/empty.png"
                ).convert("RGBA"),
                "DIRT": Image.open(
                    os.path.dirname(__file__) + "/minecraft/solid.png"
                ).convert("RGBA"),
                "path": Image.open(
                    os.path.dirname(__file__) + "/minecraft/path_g.png"
                ).convert("RGBA"),
            }
