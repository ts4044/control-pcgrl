...
def render_blocks(blocks, base_pos=5):
    ...
    block_lst = [
        Block(position=Point(x=i, y=k + 5, z=j), type=block_type, orientation=NORTH)
    ...
    for h in range(maze_height):
        for j in range(-border_size[0], 0):
            for i in range(-border_size[1], len(map[0]) + border_size[1]):
                blocks.append(
                    Block(position=Point(x=i, y=base_pos + h, z=j), type=item)
                )
        ...
        for j in range(-border_size[1], 0):
            for i in range(0, len(map)):
                blocks.append(
                    Block(position=Point(x=j, y=base_pos + h, z=i), type=item)
                )
    ...
    for j in range(len(map)):
        for i in range(len(map[j])):
            ...
            for h in range(maze_height):
                blocks.append(
                    Block(
                        position=Point(x=i, y=base_pos + h, z=j),
                        type=item,
                        orientation=NORTH,
                    )
                )
...
def spawn_2D_path(path=None, base_pos=5, item=LEAVES):
    ...
    if path:
        for pos in path:
            blocks.append(
                Block(position=Point(x=pos[0], y=base_pos, z=pos[1]), type=item)
            )
    CLIENT.spawnBlocks(Blocks(blocks=blocks))
    return
...
def spawn_3D_border(
    ...
    CLIENT.fillCube(
        FillCubeRequest(
            cube=Cube(
                min=Point(
                    x=-boundary_size - border_size[0] + 2,
                    y=base_pos - 1,
                    z=-boundary_size - border_size[1] + 2,
                ),
                max=Point(
                    x=i + boundary_size + border_size[0] - 3,
                    y=base_pos - 1,
                    z=j + boundary_size + border_size[1] - 3,
                ),
            ),
            type=backgroud_type,
        )
    ...
    CLIENT.spawnBlocks(
        Blocks(
            blocks=[
                Block(
                    position=Point(x=-1, y=base_pos - 1, z=0),
                    type=GOLD_BLOCK,
                    orientation=NORTH,
                )
            ]
    ...
    CLIENT.spawnBlocks(
        Blocks(
            blocks=[
                Block(
                    position=Point(x=i, y=base_pos + k - 3, z=j - 1),
                    type=DIAMOND_BLOCK,
                    orientation=NORTH,
                )
            ]
    ...
    for k in range(len(map)):
        for j in range(len(map[k])):
            for i in range(len(map[k][j])):
                ...
                blocks.append(
                    Block(
                        position=Point(x=i, y=k + 5, z=j), type=item, orientation=NORTH
                    )
                )
    CLIENT.spawnBlocks(Blocks(blocks=blocks))
    return
...
def spawn_3D_path(path, base_pos=5, item=TRAPDOOR):
    ...
    for pos in path:
        blocks.append(
            Block(position=Point(x=pos[0], y=pos[2] + 5, z=pos[1]), type=item)
        )
    CLIENT.spawnBlocks(Blocks(blocks=blocks))
    return
...
def edit_3D_maze(map, i, j, k, base_pos=5):
    ...
    CLIENT.spawnBlocks(
        Blocks(
            blocks=[
                Block(
                    position=Point(x=i, y=k + base_pos, z=j),
...
if __name__ == "__main__":
    ...
    CLIENT.fillCube(
        FillCubeRequest(
            cube=Cube(min=Point(x=-5, y=3, z=-5), max=Point(x=30, y=3, z=30)), type=DIRT
        )
    )
    CLIENT.fillCube(
        FillCubeRequest(
            cube=Cube(min=Point(x=-5, y=4, z=-5), max=Point(x=30, y=10, z=30)), type=AIR
        )
    )
