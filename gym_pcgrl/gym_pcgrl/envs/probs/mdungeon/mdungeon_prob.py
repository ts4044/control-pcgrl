...
class MDungeonProblem(Problem):
    ...
    def adjust_param(self, **kwargs):
        ...
        self._target_col_enemies = kwargs.get(
            "target_col_enemies", self._target_col_enemies
        )
    ...
    def get_stats(self, map):
        ...
        map_stats = {
            ...
            "regions": calc_num_regions(
                map,
                map_locations,
                ["empty", "player", "exit", "potion", "treasure", "goblin", "ogre"],
            ),
            "col-potions": 0,
            "col-treasures": 0,
            "col-enemies": 0,
        ...
        if (
            map_stats["player"] == 1
            and map_stats["exit"] == 1
            and map_stats["regions"] == 1
        ):
            map_stats["dist-win"], map_stats["sol-length"], play_stats = self._run_game(
                map
            )
    ...
    def get_reward(self, new_stats, old_stats):
        ...
        rewards = {
            ...
            "potions": get_range_reward(
                new_stats["potions"], old_stats["potions"], -np.inf, self._max_potions
            ),
            ...
            "enemies": get_range_reward(
                new_stats["enemies"], old_stats["enemies"], 1, self._max_enemies
            ),
            "regions": get_range_reward(
                new_stats["regions"], old_stats["regions"], 1, 1
            ),
            "col-enemies": get_range_reward(
                new_stats["col-enemies"], old_stats["col-enemies"], np.inf, np.inf
            ),
    ...
    def get_episode_over(self, new_stats, old_stats):
        return (
            ...
            and new_stats["enemies"] > 0
            and new_stats["col-enemies"] / max(1, new_stats["enemies"])
            > self._target_col_enemies
        )
    ...
    def render(self, map):
        if self._graphics == None:
            self._graphics = {
                "empty": Image.open(
                    os.path.dirname(__file__) + "/mdungeon/empty.png"
                ).convert("RGBA"),
                "solid": Image.open(
                    os.path.dirname(__file__) + "/mdungeon/solid.png"
                ).convert("RGBA"),
                "player": Image.open(
                    os.path.dirname(__file__) + "/mdungeon/player.png"
                ).convert("RGBA"),
                "exit": Image.open(
                    os.path.dirname(__file__) + "/mdungeon/exit.png"
                ).convert("RGBA"),
                "potion": Image.open(
                    os.path.dirname(__file__) + "/mdungeon/potion.png"
                ).convert("RGBA"),
                "treasure": Image.open(
                    os.path.dirname(__file__) + "/mdungeon/treasure.png"
                ).convert("RGBA"),
                "goblin": Image.open(
                    os.path.dirname(__file__) + "/mdungeon/goblin.png"
                ).convert("RGBA"),
                "ogre": Image.open(
                    os.path.dirname(__file__) + "/mdungeon/ogre.png"
                ).convert("RGBA"),
            }
