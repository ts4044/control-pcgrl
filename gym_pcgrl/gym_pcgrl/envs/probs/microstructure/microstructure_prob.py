...
class MicroStructureProblem(Problem):
    ...
    def get_stats(self, map, lenient_paths=False):
        ...
        self.tortuosity, self.path_length, self.path_coords = calc_tortuosity(
    ...
    def render(self, map):
        if self._graphics == None:
            if self.GVGAI_SPRITES:
                ...
                self._graphics = {
                    "empty": Image.open(
                        os.path.dirname(__file__) + "/binary/binary/empty.png"
                    ).convert("RGBA"),
                    "solid": Image.open(
                        os.path.dirname(__file__) + "/binary/binary/solid.png"
                    ).convert("RGBA"),
                    "path": Image.open(
                        os.path.dirname(__file__) + "/binary/binary/path_g.png"
                    ).convert("RGBA"),
                }
