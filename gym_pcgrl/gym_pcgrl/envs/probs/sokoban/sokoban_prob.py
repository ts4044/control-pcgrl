...
class SokobanProblem(Problem):
    ...
    def get_stats(self, map):
        ...
        if (
            map_stats["player"] == 1
            and map_stats["crate"] == map_stats["target"]
            and map_stats["crate"] > 0
    ...
    def get_reward(self, new_stats, old_stats):
        ...
        rewards = {
            ...
            "crate": get_range_reward(
                new_stats["crate"], old_stats["crate"], 1, self._max_crates
            ),
            "target": get_range_reward(
                new_stats["target"], old_stats["target"], 1, self._max_crates
            ),
            "regions": get_range_reward(
                new_stats["regions"], old_stats["regions"], 1, 1
            ),
            "ratio": get_range_reward(
                abs(new_stats["crate"] - new_stats["target"]),
            ...
            "sol-length": get_range_reward(
                len(new_stats["solution"]), len(old_stats["solution"]), np.inf, np.inf
            ),
        }
