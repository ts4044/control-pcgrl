...
def main(cfg):
    ...
    ) and ("minecraft" not in cfg.problem):
        raise Exception(
    ...
    if not cfg.load_args:
        ...
        if not cfg.overwrite:
            if os.path.isdir(log_dir):
                raise Exception(
                    "Log directory exists. Delete it first (or use command line argument `--load` to load "
    ...
    trainer_config = {
        ...
        "env_config": vars(
            cfg
        ),  # Maybe env should get its own config? (A subset of the original?)
