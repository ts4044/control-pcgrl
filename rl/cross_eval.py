    ...
    def add_key_val(key, val):
        if controllable and key != "% train archive full":
            ...
            if key in header_text:
                key = header_text[key]
            key = (
                ", ".join([c for c in stats["controls"] if c is not None]) + "||" + key
            )
...
def compile_results(settings_list, no_plot=False):
    ...
    #           keys.append(k)
    keys = ["problem", "representation", "conditionals", "alp_gmm", "change_percentage"]
    columns = None
    data = []
    vals = []
    ...
    columns = pd.MultiIndex.from_tuples(
        col_tuples, names=["", "evaluated controls", ""]
    )
    ...
    for p in ["binary", "zelda", "sokoban"]:
        ...
        p_name = p + "_ctrl"
        lcl_conds = ["---"] + [
            "-".join(pi) if len(pi) < 2 else newline(pi[0] + ", ", pi[1])
            for pi in local_controls[p_name]
        ]
        #       print(lcl_conds)
        ...
        z_cols = list(
            zip(["fixed targets"] * n_col_heads, ["---"] * n_col_heads, z_cols_fixed)
        )
        for ctrl_set in PROB_CONTROLS[p + "_ctrl"]:
            ...
            z_cols += list(
                zip(
                    ["controlled targets"] * n_col_heads,
                    [", ".join(ctrl_set)] * n_col_heads,
                    z_cols_ctrl,
                )
            )
        ...
        pandas_to_latex(
            ...
            escape=False,
            caption=(
                "Performance of controllable {} level-generating agents with learning-progress-informed (ALP-GMM) and uniform-random control regimes, and baseline (single-objective) agents, with various change percentage allowances. Agents are tested both on a baseline task with metric targets fixed at their default values, and control tasks, in which controllable metric targets are sampled over a grid.".format(
                    p
                )
            ),
            label={"tbl:{}".format(p)},
